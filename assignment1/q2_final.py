...
def load_csv(filename):
        ...
        b = numpy.ones((num_rows,1)).astype("int")
...
def margin_train_dataset(margin_w, epoch, result, label, num_cols, num_rows, learning_rate, b):
    for j in range(epoch):
        for i in range(num_rows):
            ...
            elif (ans >= -1*b) and (label[i] == 4):
                c1 = -1*b - numpy.dot(margin_w,result[i])
                c2 = learning_rate*c1*1.0
                c3 = numpy.dot(result[i], result[i])
                mm = ((c2)/c3)*(result[i])
                margin_w = numpy.add(margin_w,mm)
...
margin_w = [0.0 for i in range(num_cols)]
margin_w = numpy.array(margin_w)
epoch = 50
learning_rate = 1
